...
%open lp file
fid = fopen('HCGP2.lp', 'w');

%%%%%%%%%%%%%%%%%%%%%%%%%%%
%ESCREVER FUNÇAO OBJETIVO %
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(objind)
    coef = round(objind(i), 4); % Arredonda para 4 casas decimais
    if coef < 0
        % Para valores negativos, imprime apenas o sinal "-"
        fprintf(fid, '- %.4f x%d ', abs(coef), i);
    else
        % Para valores positivos, adiciona "+" exceto no primeiro termo
        if i == 1
...
for j = 1:length(Perms)
    ...
    fprintf(fid, ' r%d: x%d + x%d + x%d + x%d - x%d - x%d - x%d - x%d + 2 x%d <= 2\n',r, setind(2^(Sj(1)-1) + 2^(Sj(2)-1) ), setind(2^(Sj(1)-1) + 2^(Sj(4)-1)), setind(2^(Sj(2)-1) + 2^(Sj(3)-1) ), setind(2^(Sj(3)-1) + 2^(Sj(4)-1) ), setind(2^(Sj(1)-1) + 2^(Sj(2)-1)+2^(Sj(3)-1) ), setind(2^(Sj(1)-1) + 2^(Sj(2)-1)+2^(Sj(4)-1) ), setind(2^(Sj(1)-1) + 2^(Sj(3)-1)+2^(Sj(4)-1) ), setind(2^(Sj(2)-1) + 2^(Sj(3)-1)+2^(Sj(4)-1) ),setind(2^(Sj(1)-1) + 2^(Sj(2)-1)+2^(Sj(3)-1)  + 2^(Sj(4)-1)  ));               
